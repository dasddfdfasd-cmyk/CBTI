...
import { getProgressPercent, getResultFromAnswers } from './data/scoring';
...
export default function App() {
  ...
  const detailArchetype = useMemo(
    () => archetypes.find((item) => item.id === state.detailId) ?? archetypes[0],
    [state.detailId]
  );
  ...
  function startTest() {
    setState((current) => ({
      ...current,
      route: ROUTES.TEST,
      previousRoute: current.route === ROUTES.DETAIL ? ROUTES.DETAIL : ROUTES.HOME,
      currentIndex: 0,
      answers: {},
      result: null
    }));
  ...
  function handleAnswer(questionId, optionKey) {
    setState((current) => {
      ...
      const isLast = current.currentIndex === questions.length - 1;

      if (isLast) {
        return {
          ...current,
          answers: nextAnswers,
          result: getResultFromAnswers(questions, nextAnswers),
          route: ROUTES.RESULT
        };
      }
  ...
  function handleBackFromGallery() {
    setState((current) => ({
      ...current,
      route: current.previousRoute === ROUTES.RESULT || current.result ? ROUTES.RESULT : ROUTES.HOME
    }));
  }
  ...
  function handleBackFromDetail() {
    setState((current) => ({
      ...current,
      route: current.previousRoute === ROUTES.DETAIL ? ROUTES.GALLERY : current.previousRoute
    }));
  }
  ...
  return (
    <>
      ...
      {state.route === ROUTES.DETAIL && detailArchetype && (
        <ArchetypeDetailPage
